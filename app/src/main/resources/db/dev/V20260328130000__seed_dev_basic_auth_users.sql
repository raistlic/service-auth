...
using (values ('20000000-0000-0000-0000-000000000101', 'hello_world_message:read')) as source(id, name)
on target.name = source.name
when not matched then
    insert (id, name) values (source.id, source.name);

merge into permission as target
using (values ('20000000-0000-0000-0000-000000000102', 'hello_world_message:write')) as source(id, name)
on target.name = source.name
when not matched then
    insert (id, name) values (source.id, source.name);
